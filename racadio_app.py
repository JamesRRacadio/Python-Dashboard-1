...
with tab1:
    ...
    if st.button('Reload Data'):
        st.cache_data.clear()
        st.rerun()
    st.session_state.df = pd.read_excel('C:\\Users\\Racadio\\Documents\\Python_dashboard_1\\test_sheet.xlsx')
    #Format the date input.
    st.session_state.df['Date'] = pd.to_datetime(st.session_state.df['Date'], errors='coerce').dt.strftime('%m-%d-%Y')
    selection = st.multiselect('Select the Expense Type:', ['Babysitting', 'Bills', 'Food', 'Fuel', 'House'], default=[])
...
with tab2:
    with st.form('Add New Entry', clear_on_submit=True):
        type = st.selectbox('Select Expense Type:', ['Babysitting', 'Bills', 'Food', 'Fuel', 'House'])
    ...
    if submitted:
        try:
            ...
            st.session_state.df.to_excel('C:\\Users\\Racadio\\Documents\\Python_dashboard_1\\test_sheet.xlsx', index=False)
    ...
    if st.checkbox('Show current sheet'):
        st.dataframe(pd.read_excel('C:\\Users\\Racadio\\Documents\\Python_dashboard_1\\test_sheet.xlsx'))
